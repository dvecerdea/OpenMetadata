...
public class TableRepository extends EntityRepository<Table> {
  ...
  private void setDefaultFields(Table table) throws IOException {
    ...
    table.withSchema(schemaRef).withDatabase(schema.getDatabase()).withService(schema.getService());
  }
  ...
  public static String getFQN(Table table) {
    return (table != null && table.getSchema() != null)
        ? EntityNameUtil.getFQN(table.getSchema().getName(), table.getName())
        : null;
  }
  ...
  public void prepare(Table table) throws IOException {
    DatabaseSchema schema = Entity.getEntity(table.getSchema(), Fields.EMPTY_FIELDS, Include.ALL);
    table.setSchema(new DatabaseSchemaEntityInterface(schema).getEntityReference());
  ...
  public void storeRelationships(Table table) {
    ...
    addRelationship(table.getSchema().getId(), table.getId(), DATABASE_SCHEMA, TABLE, Relationship.CONTAINS);
